...
class PowerSystem:
    '''
    Classe que representa um sistema de potência.
    As suas funções permitem definir os tipo de barras, as impedancias das linhas entre as barras, os parametros de potencia ativa e reativa das barras.
    Também pode calcular a matriz Ybus do sistema e armazenar as tensoes iniciais das barras para o metodo de Gauss-Seidel.
    '''
    ...
    def add_Zline(self, bus1: int, bus2: int, Z: complex, B2: float=0) -> None:
        '''
        Adiciona uma linha de impedância entre duas barras.

        --> bus1: índice da primeira barra conectada pela linha.
        --> bus2: índice da segunda barra conectada pela linha.
        --> Z: impedância da linha em p.u. (valor complexo).
        --> B2: metade susceptância (B/2) shunt da linha em p.u. (valor real).
        '''
        ...
        # Caso o indice informado seja valido, adicionar a impedancia da linha e a susceptancia shunt da linha nos dicionarios correspondentes
    ...
    def add_bus_params(self, bus: int, Vabs: float, Vangle: float, P: float=None, Q: float=None) -> None:
        '''
        Adiciona os parametros de tensao, potencia ativa e potencia reativa para uma barra do sistema de potencia.

        --> bus: índice da barra para a qual os parâmetros estão sendo adicionados.
        --> Vabs: magnitude da tensão da barra em p.u.
        --> Vangle: ângulo da tensão da barra em graus.
        --> P: potência ativa especificada para a barra em p.u. (obrigatório para barras do tipo PQ e PV).
        --> Q: potência reativa especificada para a barra em p.u. (obrigatório para barras do tipo PQ).
        '''
        ...
        if self.bus_types[bus] == 'PQ':
            ...
            self.Pespec[bus] = P
            self.Qespec[bus] = None  # Potencia reativa nao especificada para barra PV
    ...
    def calculate_Ybus(self, verbose=False) -> None:
        '''
        Calcula a matriz Ybus do sistema de potência com base nas impedâncias das linhas entre as barras e nas susceptâncias shunt das linhas.
        Armazena os resultados na variável Ybus da classe PowerSystem.

        --> verbose: habilita a impressão da matriz Ybus calculada.
        '''
        ...
        # Calcular os elementos de Ybus fora da diagonal
        ...
        # Calcular os elementos de Ybus na diagonal
...
class GaussSeidel:
    '''
    Classe que implementa o método de Gauss-Seidel para resolver o fluxo de potência em um sistema de potência.
    As suas funções permitem realizar as iterações do método de Gauss-Seidel.
    Atualiza as tensões das barras a cada iteração e verificando a convergência com base em um critério de tolerância definido pelo usuário.
    Também pode imprimir os resultados de cada iteração, incluindo as tensões calculadas para cada barra, em um sistema de coordenadas definido pelo usuário (polar ou retangular).
    '''

    def __init__(self, sys: PowerSystem) -> None:
        self.sys = sys
        # Array que armazena as tensoes das barras para as iteracoes do metodo de Gauss-Seidel, 
        self.V_iter = sys.V0.copy()  # Inicializado com as tensoes iniciais definidas no sistema de potência

    @staticmethod
    def calc_new_V(bus: int, V_iter: np.ndarray, Ybus: np.ndarray, S_espec: complex) -> complex:
        '''
        Calcula a nova tensão para uma barra do sistema de potência com base nos argumentos passados pela função.

        --> bus: índice da barra para a qual a nova tensão está sendo calculada.
        --> V_iter: array contendo as tensões atuais das barras do sistema de potência.
        --> Ybus: matriz de admitância do sistema de potência.
        --> S_espec: potência especificada para a barra em p.u. (valor complexo, P + jQ).
        '''
    ...
    def iterate(self, tolerance: float=1e-4, max_iterations: int=100, verbose: bool=False, coordinate_system: str='polar') -> None:
        ...
        --> tolerance: Define o criterio de convergencia, ou seja, a diferenca maxima entre as tensoes das barras em iteracoes consecutivas para considerar que o metodo convergiu.
        --> max_iterations: Define o numero maximo de iteracoes a serem realizadas caso o metodo nao converja.
